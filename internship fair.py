...
bodytagsfinal = """<body>
    <div class="main">
        <img class="header" src="https://i.ibb.co/Ph7Pb4y/Banner-1.png" alt="Document">

        <div class='inside'>
            <div class="left">
                <img src="https://i.ibb.co/0qtbtPY/Past-Recruiters-1.png" alt="Image">
                <img src="https://i.ibb.co/WW7gQZS/image-3.png" alt="Image">
            </div>

            <p class='text'> Dear Manager<br>
               Greetings from Start@KMV, The Placement Cell of Keshav Mahavidyalaya, University of Delhi.<br><br>

               To continue on the path of providing novel opportunities to the students, we are organizing the <strong>second
               edition</strong> of the <strong>Virtual Internship Fair</strong> on <strong></strong>February 28,
               2021.</strong><br><br>

               Last year’s fair was an extreme success with <strong>30+ esteemed organizations</strong> offering a plethora of
               opportunities across diverse fields and witnessed more than a combined <strong>6000+ student
               applications.</strong><br><br>

               The aim of the event is to bring employers from a variety of industries — Start-ups, MNCs and Social
               Organisations — hiring for various roles of internship and apprenticeship positions. In the past,
               companies such as Sharekhan, Zomato Feeding India, Sanguine Capital, Vivo, The MoneyRoller, Grant
               Thornton, Deloitte, NITI Aayog, Wipro and many more have participated and selected students for
               internships.<br><br>

               We wish to invite your esteemed organisation for the same and look forward to a mutually beneficial
               relationship with your organisation.<br><br>

               <strong>PFA:</strong> Start@KMV Brochure<br><br>
...
def emails_to_be_sent(snum, enum, mailaddr, sheetlink, subject):
    ...
    try:
        ...
        #                starting: ending , 169, 200
            ...
            login_details= {'smtp_server': 'smtp.gmail.com', 'smtp_protocol': 'tls', 'example_no': '3', 'from_email': mailaddr, 'to_email': to_emails, 'subject': subject}
...
def get_attachment_message(from_email, to_email, subject, detailinfo, style, body, filename):
    ...
    pdf.add_header('Content-Disposition', 'attachment', filename=os.path.splitext(filename)[0].split("/")[-1] + os.path.splitext(filename)[1])
    msg.attach(pdf)
...
def send_email(email_info, detailinfo, style,body, filename, server):
    ...
    try:
        ...
        if filename == '':
            for x in to_email:
                msg = get_html_message(from_email, x, subject, detailinfo, style, body)
                server.send_message(msg)
                cli_print("Your email was sent to " + x, "green")
                lb.insert(tk.END, "Your email was sent to " + x + " Successfully \n\n")
        else:
            for x in to_email:
                msg = get_attachment_message(from_email, x, subject, detailinfo,style,body, filename)
                server.send_message(msg)
                cli_print("Your email was sent to " + x, "green")
                lb.insert(tk.END,"Your email was sent to " + x + " Successfully \n\n")
    finally:
...
if __name__ == "__main__":
    ...
    def login(*args):
        ...
        try:
            ...
            lb.insert(tk.END, "Could not connect to SMTP server with exception: %s  \n\n" % e)
    ...
    def ButtonClick(*args):
        ...
        main(startNumber,endingNumber, txtem,txtpass, sheetlink, subject, style, body, filename, server)
    ...
    window = Tk()
    window.title("Internship Fair Placement Cell")
    ...
    labelsheetlink = Label(window, text="Enter the sheet link here", fg='black', font=("karla", 14))
    labelsheetlink.pack()
    ...
    labelstarting =Label(window, text="Enter the Starting Number", fg='black', font=("karla", 14))
    labelstarting.pack()
    ...
    lblending=Label(window, text="Enter the Ending Number", fg='black', font=("karla", 14))
    lblending.pack()
    ...
    labelstyle = Label(window,text="Enter the complete style tag here", fg='black', font=("karla", 14))
    labelstyle.pack()
    ...
    labelbody = Label(window,text="Enter the complete body here", fg='black', font=("karla", 14))
    labelbody.pack()
